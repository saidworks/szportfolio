# Azure Security Services Setup Script - TEMPLATE
# This is a template file with placeholder values
# Copy this to .infra/scripts/ and replace placeholders with actual values

# INSTRUCTIONS:
# 1. Copy this file to .infra/scripts/setup-azure-security.ps1
# 2. Replace all <PLACEHOLDER> values with actual values
# 3. Never commit the file in .infra/ to version control
...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$SqlDatabaseName = "<YOUR_SQL_DATABASE_NAME>",
    
    [Parameter(Mandatory=$false)]
    [string]$FrontendAppName = "<YOUR_FRONTEND_APP_NAME>",
    
    [Parameter(Mandatory=$false)]
    [string]$ApiAppName = "<YOUR_API_APP_NAME>",
